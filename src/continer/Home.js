   ...
   // const [pageCount, setpageCount] = useState(0);
    ...
    useEffect(() => {
        const getProducts = async () => {
            try {
                const res = await axios.get("http://localhost:5000/api/homeproduct")
    ...
    //     try {
    //         const res = await axios.get(`http://localhost:5000/api/homeproduct?_page=${currentPage}&_limit=${limit}`)
    //         return res.data;
    //     } catch (data) {
    //         alert(data.error)
    //     }
